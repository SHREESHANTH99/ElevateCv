...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const resumes = await Resume.find({ userId: req.user._id })
      .sort({ updatedAt: -1 })
      .select('-personalInfo -summary -experiences -education -skills -projects');
    res.json({ resumes });
  } catch (error) {
...
router.post('/', auth, [
  ...
  body('personalInfo.fullName').optional().trim().notEmpty().withMessage('Full name cannot be empty if provided'),
  body('personalInfo.email').optional().isEmail().withMessage('Please provide a valid email'),
  body('personalInfo.phone').optional().trim().notEmpty().withMessage('Phone number cannot be empty if provided'),
  body('personalInfo.location').optional().trim().notEmpty().withMessage('Location cannot be empty if provided'),
  body('summary').optional().trim().isLength({ min: 10 }).withMessage('Summary must be at least 10 characters long if provided')
], async (req, res) => {
  try {
    ...
    if (process.env.USE_IN_MEMORY_STORAGE === 'true') {
      return res.status(400).json({
        success: false,
        message: 'Cannot save resume: Running in in-memory storage mode. Please check your MongoDB connection.'
      });
    }
    const isUpdate = req.body._id;
    let resumeData;
    if (isUpdate) {
      const existingResume = await Resume.findOne({ _id: req.body._id, userId: req.user._id });
      if (!existingResume) {
...
router.get('/export/:id?', auth, async (req, res) => {
  try {
    if (process.env.USE_IN_MEMORY_STORAGE === 'true') {
      return res.status(400).json({
        success: false,
        message: 'PDF export is not available in in-memory storage mode. Please connect to MongoDB to use this feature.'
      });
    }
    ...
    try {
      ...
      try {
        ...
        const filename = `${resume.personalInfo?.fullName?.replace(/\s+/g, '_') || 'resume'}_${new Date().toISOString().split('T')[0]}.pdf`;
...
function generateResumeHTML(resume) {
  const templateStyles = getTemplateStyles(resume.template || 'modern');
  return `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        .experience-item, .education-item, .project-item {
          margin-bottom: 20px;
          position: relative;
          ${templateStyles.itemLeftBorder ? `padding-left: 15px;
        ...
        .bullet-point {
          margin-bottom: 3px;
          position: relative;
          padding-left: ${templateStyles.bulletStyle === 'custom' ? '18px' : '0'};
        }
        ${templateStyles.bulletStyle === 'custom' ? `.bullet-point:before {
          content: '';
          position: absolute;
          left: 0;
          top: 8px;
    ...
    <body>
      <div class="header" style="${templateStyles.headerExtraStyles || ''}">
        ...
        <div class="contact-info">
          ...
          ${resume.personalInfo.linkedin ? `<span class="contact-info-item">LinkedIn: ${resume.personalInfo.linkedin}</span>` : ''}
          ${resume.personalInfo.website ? `<span class="contact-info-item">Website: ${resume.personalInfo.website}</span>` : ''}
        </div>
      </div>
      ${resume.summary ? `
        <div class="section">
            ...
            <div class="education-item">
              <div class="item-header">
                <div>
                  ...
                  ${edu.gpa ? `<div class="bullet-point">GPA: ${edu.gpa}</div>` : ''}
                </div>
                <div class="item-date">
              ...
              ${edu.description ? `<div class="description bullet-point">${edu.description}</div>` : ''}
            </div>
          `).join('')}
        </div>
      ` : ''}
        ...
        <div class="section">
          ...
          ${resume.projects.map(project => `
            <div class="project-item">
              ...
              ` : ''}
              ${project.url ? `<div class="bullet-point"><strong>URL:</strong> <a href="${project.url}" target="_blank">${project.url}</a></div>` : ''}
              ${project.github ? `<div class="bullet-point"><strong>GitHub:</strong> <a href="${project.github}" target="_blank">${project.github}</a></div>` : ''}
            </div>
          `).join('')}
        </div>
      ` : ''}
...
function getTemplateStyles(templateName) {
  const templates = {
    ...
    creative: {
      ...
      itemCompanyWeight: "600",
      dateColor: "#6b7280",
      dateStyle: "border-radius: 15px; background-color: #f3f4f6; padding: 2px 8px;",
      skillCategoryColor: "#7c3aed",
      skillCategoryWeight: "700",
      skillItemStyle: "background-color: #f3f4f6; padding: 3px 8px; border-radius: 4px; margin-bottom: 5px;",
      descriptionColor: "#4b5563",
      technologiesColor: "#6b7280",
      bulletStyle: "custom",
      headerExtraStyles: "background-color: #f9fafb; padding: 20px; border-radius: 8px;",
      itemLeftBorder: ""
    },
    minimal: {
