...
export const CardContainer = ({
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Clamp tilt angles within [-10deg, 10deg] bounds
    const clampedX = Math.max(-10, Math.min(10, y));
    const clampedY = Math.max(-10, Math.min(10, -x));

    containerRef.current.style.transform = `rotateX(${clampedX}deg) rotateY(${clampedY}deg)`;
  };
...
export const useCardContext = () => {
  ...
  if (context === undefined) {
    throw new Error("useCardContext must be used within a CardContainer");
  }
  return context;
};
